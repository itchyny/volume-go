...
// GetMicrophoneVolume returns the current volume (0 to 100).
func GetMicrophoneVolume() (int, error) {
	vol, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		var level float32
		err := aev.GetMasterVolumeLevelScalar(&level)
		vol := int(math.Floor(float64(level*100.0 + 0.5)))
		return vol, err
...
// SetMicrophoneVolume sets the sound volume to the specified value.
func SetMicrophoneVolume(volume int) error {
	if volume < 0 || 100 < volume {
		return errors.New("out of valid volume range")
	}
	_, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		err := aev.SetMasterVolumeLevelScalar(float32(volume)/100, nil)
		return nil, err
	})
	return err
...
// IncreaseMicrophoneVolume increases (or decreases) the audio volume by the specified value.
func IncreaseMicrophoneVolume(diff int) error {
	_, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		var level float32
		err := aev.GetMasterVolumeLevelScalar(&level)
		if err != nil {
...
// GetMicrophoneMuted returns the current muted status.
func GetMicrophoneMuted() (bool, error) {
	muted, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		var muted bool
		err := aev.GetMute(&muted)
		return muted, err
	})
...
// MicrophoneMute mutes the audio.
func MicrophoneMute() error {
	_, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		err := aev.SetMute(true, nil)
		return nil, err
	})
...
// MicrophoneUnmute unmutes the audio.
func MicrophoneUnmute() error {
	_, err := invokeMicrophone(func(aev *wca.IAudioEndpointVolume) (interface{}, error) {
		err := aev.SetMute(false, nil)
		return nil, err
	})
...
func invokeMicrophone(f func(aev *wca.IAudioEndpointVolume) (interface{}, error)) (ret interface{}, err error) {
	if err = ole.CoInitializeEx(0, ole.COINIT_APARTMENTTHREADED); err != nil {
		return
	}
	defer ole.CoUninitialize()
